...
class Munc13:
    ...
    def munc13_hexamerOde_loopCa(self, t, y, params):
        ...
        This is CHANGED from munc13_hexamerOde_loopCa by the addition of a new membrane specie C
        ...
        1-19
          ...
          ** 19)	D2X3+D<->D3X3  gamma*kfmx, krmx*exp(3dGC+eLoop) #this can add 2 or 3 dGC
           ...
           ** 24)	D2MX3+M <->D2M2X3 gamma*kfc, krc*exp(2dGC)
            ...
            35) DX+DX <->D2X2 dGXX: gamma*kfxx, krxx*dGCH
            36) M2X+M2X <->M4X2 dGXX: gamma*kfxx, krxx
            37) D2X2+DX <->D3X3 2dGXX+2dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)*DdGCH
            ...
            41) DM2X2 + DX <-> D2M2X3 2*dGXX+2*dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)*dCHG
        ...
        r35b = y[15]*krxx*chgDG #chgDG is only when D is involved, single vs 2 or 3.
        ...
        dylist.append(+r2-r2b-r7+r7b-r17+r17b-r19+r19b-r43+r43b)#3 D
    ...
    def simulate_pre(self, candidate):
        ...
        """
        numVar = 27 #number of variables we are tracking. 
        ...
        rateParams=np.array([kfsr, krsr, kfmm, krmm, kfmx, krmx, kfc, krc, kfxx, krxx, eLoop, eDF, kfdd, kfcm, krcm, eCA, kclus])
    ...
    def simulate_post(self, candidate, solution_pre):
        ...
        rateParams=np.array([kfsr*stimUpSR, krsr, kfmm, krmm, kfmx, krmx, kfc, krc, kfxx, krxx, eLoop, eDF, kfdd, kfcm, krcm, eCA, kclus])
...
class Solver:
    ...
    def write_sortedParms(self, viablePoints, viableFitness):
        ...
        with open(outfile, "w") as f:
            # CSV header
            header = (
                "Rank, Fitness, kfsr, krsr, kfmm, krmm, kfmx, krmx, kfc, krc, kfxx, krxx, eLoop, eDF, kfdd, stimUpSR, S0, R0, X0, kfcm, krcm, eCA, kclus\n"
            )
            f.write(header)
            ...
            for rank, idx in enumerate(idx_sorted):
                ...
                f.write(
                    ...
                    f"{S0:.5g}, {R0:.5g}, {X0:.5g}, "
                    f"{kfcm:.5g},{krcm:.5g},{eCA:.5g},{kclus:.5g}\n"
                )
...
if __name__ == "__main__":
    ...
    parameter_ranges = {
       ...
       # "kfc_nostim":      {"min": 0.001, "max": 10},   # kfc_nostim
        ...
        "eLoop":      {"min": 0.001,   "max": 10}, # exp(free energy kT units <0).
    ...
    params_to_optimizeCa = np.array([
        "kfsr","krsr","kfmm","krmm","kfmx","krmx","kfc","krc","kfxx","krxx","eLoop","eDF","kfdd","stimUpSR","S0","R0","X0","C0","kfcm", "krcm","eCA","kclus"
    ])
