...
class Munc13:
    ...
    def munc13_hexamerOde(self, t, y, params):
        """
        ...
        dGSR: kfsr, krsr is for recruitment to the membrane
        dGM: kfmm, krmm is for dimerization of munc13
        dGMX: kfmx, krmx is for binding to the cluster nucleator X of a monomer
        dGC: kfc, krc is for binding of a monomer to the cluster nucleator X with a monomer already present
        
        dGXX: kfxx, krxx is the rate of adding another nucleator X to the cluster
        eDF: kfdd, krdd (with other dGs)
        krmxDimer is the off-rate of X from the dimer complex. It has
        a rate of dGC and eDF are both negative, so off rate is reduced.
        krmxDimer=krmx*S*exp(dGC+eDF)
        kfast=max(kfc, kfmx)
        krfcmx = kfast*c0exp(dgMX+dgC)
                Reaction equations:
        1-19
            1. S + R <-> M, kfsr, krsr
            ...
            7. D + X <-> DX, gamma*kfmx*S, krmxDimer
            8. M2X<->DX kfDD, krDD dG=dMM+eDF
            9.  X + M2X <-> M2X2, gamma*kfxx, krxx
            10. DX+ X <->DX2, gamma*S*kfxx, S*krxx
            11. M2X2 <-> DX2, kfDD, krDD
            12. M2X2+M <-> M3X2 gamma*kfc, krc
            13. DX2+M <-> DMX2 gamma*kfc, krc
            14. M3X2+M <->M4X2 gamma*kfast, krfcmx
            15)	DMX2+M <->DM2X2 gamma*kfast, krfcmx
            16)	DM2X2 <->D2X2 kfDD, krDD
            17)	DX2+D<->D2X2 gamma*kfast, krfcmx*exp(eDF+dGC) (slower than r15) 
            18)	D2X2+X <-> D2X3 gamma*S*kfxx, S*krxx*exp(dGX)
            19)	D2X3+D<->D3X3  gamma*kfast, krfcmx*exp(2*dGC+eDF) 
            20)	M4X2+X <->M4X3 gamma*kfxx, krxx*exp(dGXX) 
            21)	M4X3+M <->M5X3 gamma*kfc, krc
            22)	M5X3+M <->M6X3 gamma*kfast, krfcmx*exp(dGC)
            23)	D2X3+M <->D2MX3 gamma*kfc, krc
            24)	D2MX3+M <->D2M2X3 gamma*kfc, krc*exp(dGC+dGMX)
            25)	D2M2X3<->D3X3 kfDD, krDD
            26)	S+R+MX->M2X  gamma*kfc*kfsr/krsr
            27)	S+R+M2X2->M3X2 gamma*kfc*kfsr/krsr
            28)	S+R+DX2 ->DMX2 gamma*kfc*kfsr/krsr
            29)	S+R+M3X2->M4X2 gamma*kfast*kfsr/krsr
            30)	S+R+DMX2->DM2X2 gamma*kfast*kfsr/krsr
            31)	S+R+M4X3->M5X3 gamma*kfc*kfsr/krsr
            32)	S+R+D2X3->D2MX3 gamma*kfc*kfsr/krsr
            33)	S+R+M5X3->M6X3 gamma*kfast*kfsr/krsr
            34)	S+R+D2MX3->D2M2X3 gamma*kfast*kfsr/krsr
        ...
        r6b = krc*y[7]
        r7 = gamma*kfmx*sig*y[3]*y[4]
        r7b = krmxDimer*y[8]
        r8 = kfdd*y[7]
        r8b = krdd*y[8]
        r9 = gamma*kfxx*y[4]*y[7]
        r9b = krxx*y[9]
        r10 = gamma*sig*kfxx*y[4]*y[8]   
        r10b = sig*krxx*y[10]
        r11 = kfdd*y[9]
        r11b = krdd*y[10]
        r12 = gamma*kfc*y[9]*y[2]
        r12b = krc*y[11]
        r13 = gamma*kfc*y[2]*y[10]
        r13b = krc*y[12]
        r14 = gamma*kfast*y[2]*y[11]
        r14b = kr_cmx*y[13]
        r15 = gamma*kfast*y[2]*y[12]
        r15b = kr_cmx*y[14]
        r16 = kfdd*y[14]
        r16b = krdd*y[15]
        r17 = gamma*kfast*y[10]*y[3]
        r17b = kr_cmx*np.exp(dGC)*eDF*y[15]
        r18 = gamma*sig*kfxx*y[15]*y[4]
        r18b = sig*krxx*np.exp(dGXX)*y[17]
        r19 = gamma*kfast*y[17]*y[3]
        r19b = kr_cmx*np.exp(2*dGC)*eDF*y[22]
        r20 = gamma*kfxx*y[13]*y[4]
        r20b = krxx*np.exp(dGXX)*y[16]
        r21 = gamma*kfc*y[16]*y[2]
        r21b = krc*y[18]
        r22 = gamma*kfast*y[2]*y[18]
        r22b = kr_cmx*np.exp(dGC)*y[20]
        r23 = gamma*kfc*y[17]*y[2]
        r23b = krc*y[19]
        r24 = gamma*kfc*y[19]*y[2]
        r24b = krc*np.exp(dGC+dGMX)*y[21]
        r25 = kfdd*y[21]
        r25b = krdd*y[22]
        r26 =  gamma*kfc*kfsr/krsr*y[0]*y[1]*y[5]
        r27 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[9]
        r28 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[10]
        r29 = gamma*kfast*kfsr/krsr*y[0]*y[1]*y[11]
        r30 = gamma*kfast*kfsr/krsr*y[0]*y[1]*y[12]
        r31 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[16]
        r32 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[17]
        r33 = gamma*kfast*kfsr/krsr*y[0]*y[1]*y[18]
        r34 = gamma*kfast*kfsr/krsr*y[0]*y[1]*y[19]

        dylist = []
        #add each new species reaction rate.
        dylist.append(-r1+r1b-r4+r4b-r26-r27-r28-r29-r30-r31-r32-r33-r34)#0 S
        dylist.append(-r1+r1b-r5+r5b-r26-r27-r28-r29-r30-r31-r32-r33-r34)#1 R
        dylist.append(+r1-r1b-2*r2+2*r2b-r3+r3b-r6+r6b-r12+r12b-r13+r13b-r14+r14b-r15+r15b-r21+r21b-r22+r22b-r23+r23b-r24+r24b)#2 M
        dylist.append(+r2-r2b-r7+r7b-r17+r17b-r19+r19b)#3 D
        ...
        dylist.append(r6-r6b-r8+r8b-r9+r9b+r26)#7 M2X
        dylist.append(r7-r7b+r8-r8b-r10+r10b)#8 DX
        ...
        dylist.append(r14-r14b-r20+r20b+r29)#13 M4X2
        dylist.append(r15-r15b-r16+r16b+r30)#14 DM2X2
        dylist.append(r16-r16b+r17-r17b-r18+r18b)#15 D2X2
        ...
        dylist.append(r22-r22b+r33) #20 M6X3
        dylist.append(r24-r24b-r25+r25b+r34) #21 D2M2X3
        dylist.append(r19-r19b+r25-r25b) #22 D3X3
    ...
    def munc13_hexamerOde_allPaths(self, t, y, params):
        """
        Maggie Johnson, Oct 2025
        Munc13 model with explicit clustering. 
        We will no longer enforce a specific cluster regime, so we want the
        ...
        1-19
            ...
            9.  X + M2X <-> M2X2, gamma*kfxx, krxx
            10. DX+ X <->DX2, gamma*S*kfxx, S*krxx
            11. M2X2 <-> DX2, kfDD, krDD (uses eDF)
           * 12. M2X2+M <-> M3X2 gamma*kfmx, krmx
           * 13. DX2+M <-> DMX2 gamma*kfmx, krmx
           * 14. M3X2+M <->M4X2 gamma*kfc, krc*exp(dGC) #adds an extra dGC
           * 15)	DMX2+M <->DM2X2 gamma*kfc, krc*exp(dGC) #adds an extra dGC
           ! 16)	DM2X2 <->D2X2 kfDD, krDD2
           !* 17)	DX2+D<->D2X2 gamma*kfmx, krmx*exp(eLoop+2dGC) (slower than r15) 
            18)	D2X2+X <-> D2X3 gamma*S*kfxx, S*krxx*exp(dGX)
           !* 19)	D2X3+D<->D3X3  gamma*kfmx, krmx*exp(3dGC+eLoop) 
            20)	M4X2+X <->M4X3 gamma*kfxx, krxx*exp(dGXX) 
            *21)	M4X3+M <->M5X3 gamma*kfmx, krmx
            *22)	M5X3+M <->M6X3 gamma*kfc, krc*exp(2dGC)
            *23)	D2X3+M <->D2MX3 gamma*kfmx, krmx
            *24)	D2MX3+M <->D2M2X3 gamma*kfc, krc*exp(2dGC)
           ! 25)	D2M2X3<->D3X3 kfDD, krDD2
            26)	S+R+MX->M2X  gamma*kfc*kfsr/krsr
            ...
            !35) DX+DX <->D2X2 dGXX+dGC: gamma*S*kfxx, S*krxx*exp(dGC)*exp(-eDF+eLoop)
            36) M2X+M2X <->M4X2 dGXX+dGC: gamma*kfxx, krxx*exp(dGC)
            !37) D2X2+DX <->D3X3 2dGXX+2dGC: gamma*S*kfxx, S*krxx*exp(dGX+2*dGC)*exp(-eDF+eLoop)
            38) M4X2 + M2X <->M6X3 2dGXX + 2dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)
            #Include mixtures of D and M
            39) DX + M2X <->DM2X2 dGXX+dGC: gamma*kfxx, krxx*exp(dGC)
            40) D2X2+M2X <->D2M2X3: 2*dGXX+2*dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)
            !41) DM2X2 + DX <-> D2M2X3 2*dGCC+2*dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)*exp(-eDF+eLoop)
        ...
        dGXX = np.log(krxx*1e-6/kfxx) #K_D in units of M
        krdd = kfdd*np.exp(dGM)*eDF  #the ratio of on/off for the 2M<->D transition is controlled by other free energies
        krdd2 = kfdd*np.exp(dGM)*eLoop #only the first D addition uses eDF.
        chgDG=eLoop/eDF #replace the eDF stabilization with the eLoop stabilization
        #krxTrimer = params[12]
        #enhanceRate = 10 #scalare that increase the on-rate when more than one X is present
        #krmxDimer=krmx*sig*np.exp(dGC)*eDF # off rate for MX slowed by C and F=dG
        #kfast=max(kfc, kfmx)  #two interfaces, choose faster rate
        ...
        #leave stoichiometric factors for the dy expressions
        #change such that eDF only affects the first dimer addition.
        #subsequent dimer additions use eLoop
        r1 = kfsr*y[0]*y[1] #in 3D
        r1b = krsr*y[2]
        r2 = gamma*kfmm*y[2]*y[2]
        r2b = krmm*y[3]
        ...
        r14b = krc*np.exp(dGC)*y[13]
        r15 = gamma*kfc*y[2]*y[12]
        r15b = krc*np.exp(dGC)*y[14]
        r16 = kfdd*y[14]
        r16b = krdd2*y[15]
        r17 = gamma*kfmx*y[10]*y[3]
        r17b = krmx*np.exp(2*dGC)*eLoop*y[15]
        r18 = gamma*kfxx*y[15]*y[4]
        r18b = krxx*np.exp(dGXX)*y[17]
        r19 = gamma*kfmx*y[17]*y[3]
        r19b = krmx*np.exp(3*dGC)*eLoop*y[22]
        r20 = gamma*kfxx*y[13]*y[4]
        r20b = krxx*np.exp(dGXX)*y[16]
        r21 = gamma*kfmx*y[16]*y[2]
        r21b = krmx*y[18]
        r22 = gamma*kfc*y[2]*y[18]
        r22b = krc*np.exp(2*dGC)*y[20]
        r23 = gamma*kfmx*y[17]*y[2]
        r23b = krmx*y[19]
        r24 = gamma*kfc*y[19]*y[2]
        r24b = krc*np.exp(2*dGC)*y[21]
        r25 = kfdd*y[21]
        r25b = krdd2*y[22]
        r26 =  gamma*kfc*kfsr/krsr*y[0]*y[1]*y[5]
        r27 = gamma*kfmx*kfsr/krsr*y[0]*y[1]*y[9]
        r28 = gamma*kfmx*kfsr/krsr*y[0]*y[1]*y[10]
        r29 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[11]
        r30 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[12]
        r31 = gamma*kfmx*kfsr/krsr*y[0]*y[1]*y[16]
        r32 = gamma*kfmx*kfsr/krsr*y[0]*y[1]*y[17]
        r33 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[18]
        r34 = gamma*kfc*kfsr/krsr*y[0]*y[1]*y[19]
        r35 = gamma*kfxx*y[8]*y[8]
        r35b = y[15]*krxx*np.exp(dGC)*chgDG
        r36 = gamma*kfxx*y[7]*y[7]
        r36b = y[13]*krxx*np.exp(dGC)
        r37 = gamma*kfxx*y[8]*y[15]
        r37b = y[22]*krxx*np.exp(dGXX+2*dGC)*chgDG
        r38 = gamma*kfxx*y[7]*y[13]
        r38b = y[20]*krxx*np.exp(dGXX+2*dGC)
        r39 = gamma*kfxx*y[8]*y[7]
        r39b = y[14]*krxx*np.exp(dGC)
        r40 = gamma*kfxx*y[15]*y[7]
        r40b = y[21]*krxx*np.exp(dGXX+2*dGC)
        r41 = gamma*kfxx*y[8]*y[14]
        r41b = y[21]*krxx*np.exp(dGXX+2*dGC)*chgDG
        ...
        dylist.append(r21-r21b-r22+r22b-r33+r31)#18 M5X3
        dylist.append(r23-r23b-r24+r24b-r34+r32)#19 D2MX3
        dylist.append(r22-r22b+r33+r38-r38b) #20 M6X3
        dylist.append(r24-r24b-r25+r25b+r34+r40-r40b+r41-r41b) #21 D2M2X3
        dylist.append(r19-r19b+r25-r25b+r37-r37b) #22 D3X3

        return np.array(dylist)
    "END OF HEXAMER ODE ALL PATHS"

    def munc13_hexamerOde_loop(self, t, y, params):
        """
        Maggie Johnson, Oct 2025
        Munc13 model with explicit clustering. 
        We will no longer enforce a specific cluster regime, so we want the
        ...
        1-19
          ...
          ** 15)	DMX2+M <->DM2X2 gamma*kfc, krc
            16)	DM2X2 <->D2X2 kfDD, krDD
          ** 17)	DX2+D<->D2X2 gamma*kfmx, krmx*exp(eLoop+1dGC) (slower than r15) 
            18)	D2X2+X <-> D2X3 gamma*S*kfxx, S*krxx*exp(dGX)
          ** 19)	D2X3+D<->D3X3  gamma*kfmx, krmx*exp(3dGC+eLoop) #this can add 2 or 3 dGC
            20)	M4X2+X <->M4X3 gamma*kfxx, krxx*exp(dGXX) 
            21)	M4X3+M <->M5X3 gamma*kfmx, krmx
           ** 22)	M5X3+M <->M6X3 gamma*kfc, krc*exp(2dGC)
            23)	D2X3+M <->D2MX3 gamma*kfmx, krmx
           ** 24)	D2MX3+M <->D2M2X3 gamma*kfc, krc*exp(2dGC)
            ...
            34)	S+R+D2MX3->D2M2X3 gamma*kfc*kfsr/krsr
            #ALLOW X to add onto clusters even if monomers are attached.
            #If only the X bond forms, no extra dGC, until loop closure
            35) DX+DX <->D2X2 dGXX+dGC: gamma*kfxx, krxx
            36) M2X+M2X <->M4X2 dGXX+dGC: gamma*kfxx, krxx
            37) D2X2+DX <->D3X3 2dGXX+2dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)
            38) M4X2 + M2X <->M6X3 2dGXX + 2dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)
            #Include mixtures of D and M
            39) DX + M2X <->DM2X2 dGXX+dGC: gamma*kfxx, krxx
            40) D2X2+M2X <->D2M2X3: 2*dGXX+2*dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)
            41) DM2X2 + DX <-> D2M2X3 2*dGCC+2*dGC: gamma*kfxx, krxx*exp(dGX+2*dGC)
        ...
        r16b = krdd2*y[15]
        r17 = gamma*kfmx*y[10]*y[3]
        r17b = krmx*np.exp(dGC)*eLoop*y[15]
        r18 = gamma*kfxx*y[15]*y[4]
        r18b = krxx*np.exp(dGXX)*y[17]
        r19 = gamma*kfmx*y[17]*y[3]
        r19b = krmx*np.exp(3*dGC)*eLoop*y[22]
        ...
        return np.array(dylist)
    "END OF HEXAMER ODE loop"

    def munc13_testOde(self, t, y, params):
        """
        ...
        kfast=max(kfc, kfmx)  #two interfaces, choose faster rate
        kr_cmx = kfast*np.exp(dGMX+dGC) #reverse reaction with MX and C bonds=dG
        #print out all computed parameters.
        #print("dGM ", dGM)
        #print("dGC ", dGC)
        #print("dGMX ", dGMX)
        #print("krdd ", krdd)
        #print("krmxDimer, ", krmxDimer)
        #print("kfast, ", kfast)
        #print("kr_cmx ", kr_cmx)
        ...
        r17b = kr_cmx*np.exp(dGC)*eDF*y[15]
        r18 = gamma*sig*kfxx*y[15]*y[4]
        r18b = sig*krxx*np.exp(dGM+dGXX)*y[17]
        r19 = gamma*kfast*y[17]*y[3]
        r19b = kr_cmx*np.exp(dGC)*eDF*y[22]
        r20 = gamma*kfxx*y[13]*y[4]
        r20b = krxx*np.exp(dGXX)*y[16]
        r21 = gamma*kfast*y[16]*y[2]
        r21b = kr_cmx*np.exp(dGC)*eDF*y[18]
        r22 = gamma*kfast*y[2]*y[18]
        r22b = kr_cmx*np.exp(dGC)*eDF*y[20]
        r23 = gamma*kfc*y[17]*y[2]
        r23b = krc*y[19]
        r24 = gamma*kfc*y[19]*y[2]
        r24b = krc*np.exp(dGC+dGMX)*y[21]
        ...
        r25b = krdd*y[22]
        r26 = 0 #gamma*kfc*kfsr/krsr*y[0]*y[1]*y[5]
        r27 = 0 #gamma*kfc*kfsr/krsr*y[0]*y[1]*y[9]
        r28 = 0 #gamma*kfc*kfsr/krsr*y[0]*y[1]*y[10]
        r29 = 0 #gamma*kfast*kfsr/krsr*y[0]*y[1]*y[11]
        r30 = 0 #gamma*kfast*kfsr/krsr*y[0]*y[1]*y[12]
        r31 = 0 #gamma*kfc*kfsr/krsr*y[0]*y[1]*y[16]
        r32 = 0 #gamma*kfc*kfsr/krsr*y[0]*y[1]*y[17]
        r33 = 0 #gamma*kfast*kfsr/krsr*y[0]*y[1]*y[18]
        r34 = 0 #gamma*kfast*kfsr/krsr*y[0]*y[1]*y[19]
       ...
       #add each new species reaction rate.
        ...
        dylist.append(r3-r3b+r5-r5b-r6+r6b-r26)#5 MX
        dylist.append(r4-r4b-r5+r5b)#6 SX
        dylist.append(r6-r6b-r8+r8b-r9+r9b)#7 M2X
        dylist.append(r7-r7b+r8-r8b-r10+r10b)#8 DX
        dylist.append(r9-r9b-r11+r11b-r12+r12b-r27)#9 M2X2
        dylist.append(r10-r10b+r11-r11b-r13+r13b-r17+r17b-r28)#10 DX2
        dylist.append(r12-r12b-r14+r14b-r29)#11 M3X2
        dylist.append(r13-r13b-r15+r15b-r30)#12 DMX2
        dylist.append(r14-r14b-r20+r20b)#13 M4X2
        dylist.append(r15-r15b-r16+r16b)#14 DM2X2
        dylist.append(r16-r16b+r17-r17b-r18+r18b)#15 D2X2
        dylist.append(r20-r20b-r21+r21b-r31)#16 M4X3
        dylist.append(r18-r18b-r19+r19b-r23+r23b-r32)#17 D2X3
        dylist.append(r21-r21b-r22+r22b-r33)#18 M5X3
        dylist.append(r23-r23b-r24+r24b-r34)#19 D2MX3
        dylist.append(r22-r22b) #20 M6X3
        dylist.append(r24-r24b-r25+r25b) #21 D2M2X3
        dylist.append(r19-r19b+r25-r25b) #22 D3X3
    ...
    def costChi_cluster(self, Y, densExpt):
        ...
        #implement a relu penalty if percent in clusters is less than 10%
        chiPercClust =chiPercClust + max(0, 0.1 - percClustMem)*-weightPerc
    ...
    def calculate_cluster_density(self,Y):
        ...
        cluster_copies = copies[20]+copies[21]+copies[22]+copies[18]+copies[19]
    ...
    def costChi_recruitmentStim(self, Ypre, Ypost, observedStim):
        ...
        recruitStim=memDensMuncPost/memDensMuncPre
        ...
        print(f"Recruitment dens pre {memDensMuncPre}, and dens post {memDensMuncPost}")
    ...
    def simulate_pre(self, candidate):
        ...
        rateParams=np.array([kfsr, krsr, kfmm, krmm, kfmx, krmx, kfc, krc, kfxx, krxx, eLoop, eDF, kfdd])
        ...
        solution = scipy.integrate.solve_ivp(fun=self.munc13_hexamerOde_loop, method='BDF', t_span=(0, self.t_max), y0=initValues, t_eval=self.timePoints, args=(rateParams,), rtol=1e-7, atol=1e-9)
    ...
    def simulate_post(self, candidate, solution_pre):
        ...
        rateParams=np.array([kfsr*stimUpSR, krsr, kfmm, krmm, kfmx, krmx, kfc, krc, kfxx, krxx, eLoop, eDF, kfdd])
    ...
    def test_mass_conservation(self, Y):
        ...
        muncVsTime=copies[0]+copies[2]+2*copies[3]+\
                    copies[5]+copies[6]+2*copies[7]+\
                    2*copies[8]+2*copies[9]+2*copies[10]+\
                    3*copies[11]+3*copies[12]+\
                    4*copies[13]+4*copies[14]+4*copies[15]+\
                    4*copies[16]+4*copies[17]+5*copies[18]+\
                    5*copies[19]+6*copies[20]+6*copies[21]+6*copies[22]
        #calculate the total number of R in the system at each time point
        RvsTime=copies[1]+muncVsTime-copies[0]-copies[6] 
        ...
        XvsTime=copies[4]+copies[5]+copies[6]+copies[7]+copies[8]+2*copies[9]+\
                    ...
                    3*copies[19]+3*copies[20]+3*copies[21]+3*copies[22]
    ...
    def calc_percentages_cluster(self, solution):
        ...
        clusterCopies=6*(copies[20]+copies[21]+copies[22])+5*(copies[18]+copies[19])
        memDens=self.calculate_munc13_on_membrane(copies)
        memCopies=memDens*self.cellArea
        muncTotal=copies[0]+copies[2]+2*copies[3]+\
                    copies[5]+copies[6]+2*copies[7]+\
                    2*copies[8]+2*copies[9]+2*copies[10]+\
                    3*copies[11]+3*copies[12]+\
                    4*copies[13]+4*copies[14]+4*copies[15]+\
        ...
        pMono = copies[2]/memCopies
        pDimer = 2*copies[3]/memCopies
...
class Solver:
    ...
    def write_sortedParms(self, viablePoints, viableFitness):
        ...
        with open(outfile, "w") as f:
            # CSV header
            header = (
                "Rank, Fitness, kfsr, krsr, kfmm, krmm, kfmx, krmx, kfc, krc, kfxx, krxx, eLoop, eDF, kfdd, stimUpSR, S0, R0, X0\n"
            )
            f.write(header)
...
if __name__ == "__main__":
    ...
    parameter_ranges = {
       ...
       # "kfc_nostim":      {"min": 0.001, "max": 10},   # kfc_nostim
        ...
        "eLoop":      {"min": 0.001,   "max": 1}, # exp(free energy kT units <0).
    ...
    filename = f"../data/testParms_StimOnly_{random_number}.txt"
